        ...
        if format == "json":
            ...
            for sheet in content:
                docs.append(Document(
                    page_content=XMindLoader.topic2md_json(sheet["rootTopic"], is_root=True), 
                    metadata={"source": self.file_path, "title": sheet["title"]}
                ))
            ...
            for sheet in content.findall("sheet"):
                docs.append(Document(
                    page_content=XMindLoader.topic2md_xml(sheet.find("topic"), is_root=True), 
                    metadata={"source": self.file_path, "title": sheet.find("title").text}
                ))
